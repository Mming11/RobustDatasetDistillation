...
from new_strategy import NEW_Strategy, Cluster_Strategy, cluster_loss
class Config:
    ...
    test = [452, 64, 374, 236, 993, 176, 882, 904, 503, 74]

    #test = [193, 182, 258, 162, 155, 176, 882, 904, 503, 74]
...
def add_noise(image):
    ...
    noise = np.random.randint(0, 256, (3, width, height))
    noisy_image = np.add(noisy_image, np.float32(noise), dtype=np.float32)
 
    return noisy_image

"""
You can load different processed datasets, such as invert, noise
Here, the loaded data set is the original data set
"""
def get_robust_data(dataset, data_path, batch_size=1, res=None, args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        dst_test = datasets.SVHN(data_path,
                                    split='test',
                                    download=True,
                                    transform=transform)
        ...
        list_idx = np.squeeze(np.argwhere(np.isin(dst_test.targets, config.img_net_classes)))
...
def get_dataset(dataset, data_path, batch_size=1, res=None, args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.SVHN(data_path, split='train',
                                      download=True,
...
class TensorDataset_cvar(Dataset):
    def __init__(self, images, labels, klabels): # images: n x c x h x w tensor
        self.images = images.detach().float()
        self.labels = labels.detach()
        self.klabels = klabels.detach()

    def __getitem__(self, index):
        return self.images[index], self.labels[index], self.klabels[index]
...
def cvar_loss(loss, alpha=0.9):  
    """  
    Calculate the CVaR loss function
    :param loss: indicates the loss matrix
    :param alpha: The alpha parameter of CVaR
    :return: CVaR loss matrix
    """  
      
    # Sort the losses and get the sorted index
    _, sorted_indices = torch.sort(loss, dim=0, descending=False)  
    sorted_loss = loss[sorted_indices]  
   
    # Calculate the index range of CVaR losses
    cvar_range = int(alpha * loss.size(0))  
    if cvar_range == 0 and loss.size(0) != 0:
        cvar_range = 1
    
    # Calculate the CVaR loss matrix
    cvar_losses = (non_zero_mean(sorted_loss[:cvar_range]) + sorted_loss[cvar_range - 1]*(alpha - cvar_range/loss.size(0)))/alpha 

    return cvar_losses

def non_zero_mean(tensor):
    tensor = tensor.float()  # Converts the tensor to float type
    non_zero_elements = tensor[tensor != 0]  # Select all non-zero elements
    if len(non_zero_elements) > 0:
        return non_zero_elements.mean()  # Calculates the mean of the nonzero elements
    else:
        return 0   # Returns 0 if all elements are zero

def epoch_mix(mode, trainloader, net, optimizer, criterion, criterion1, num_classes, args, aug):
    ...
    num = 0
    cvarloss_list = [[] for c in range(num_classes*args.batch_kcentral)]
  
    for i_batch, datum in enumerate(trainloader):
        
        img = datum[0].to(args.device)
        lab = datum[1].to(args.device)
        label_index = datum[2].to(args.device)
        ...
        loss1 = criterion1(output, lab)

        predicted = torch.argmax(output.data, 1)
        correct = (predicted == lab).sum()
       
        loss_avg += loss1.item()*n_b
        ...
        if mode == 'train':
            ...
            for j in range(len(loss)):
                ...
                cvarloss_list[label_index[j]].append(loss[j])

            if i_batch % args.usefrequency == 0:
                
                cvar_losses = torch.zeros(num_classes*args.batch_kcentral, 1)
                list_mid = np.zeros([len(cvarloss_list),len(max(cvarloss_list,key = lambda x: len(x)))])
                for i,j in enumerate(cvarloss_list):
                    for m in range(len(j)):
                        list_mid[i][m] = j[m].cpu()

                cvar_tensor = torch.tensor(list_mid).to("cuda:0")    
    
                for i in range(len(cvarloss_list)):  
                    loss_row = cvar_tensor[i]  
                    cvar_loss1 = cvar_loss(loss_row, alpha=args.alpha)  
                    cvar_losses[i] = cvar_loss1  
     
                average = (non_zero_mean(cvar_losses)).requires_grad_(True)  
                max_element = torch.max(cvar_losses).requires_grad_(True)  
    ...
    return loss_avg, acc_avg, loss_cvar


def epoch_baseline(mode, dataloader, net, optimizer, criterion, args, aug):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(args.device)
    # criterion = criterion.to(args.device)
...
def epoch_withno_embeddings(mode, dataloader, net, optimizer, criterion, args, aug):
    loss_avg, acc_avg, num_exp = 0, 0, 0
    net = net.to(args.device)
    # criterion = criterion.to(args.device)
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if "imagenet" in args.dataset:
            if mode == 'robust_test':
                ...
                num_noise_images = int(noise_ratio * img.shape[0])

                for i in range(num_noise_images):
                    index = random.randint(0, img.shape[0]-1)
                    channel, width, height = img.shape[1:]
  
                    #noise = np.random.randint(0, 256, (channel, width, height))
                    noise = np.random.randint(0, 256, (channel, width, height))
        
                    img[index] = (transforms.ToTensor()(noise)).transpose(0, 1).transpose(1, 2).to(args.device)
...
def evaluate_synset(it_eval, net, images_train, labels_train, robust_testloader, testloader, args, decay="cosine", return_loss=False, test_it=100, aug=True):
    net = net.to(args.device)
    ...
    robust_acc_train_list = []
    robust_loss_train_list = []
    robust_acc_test_list = []
    robust_loss_test_list = []
    robust_acc_test_max = 0
    robust_acc_test_max_epoch = 0


    cluster_loss_min_list = []
    cluster_loss_mean_list = []
    for ep in tqdm.tqdm(range(Epoch)):
        loss_train, acc_train = epoch_withno_embeddings('train', trainloader, net, optimizer, criterion, args, aug=aug)
    ...
    with torch.no_grad():
        loss_test, acc_test = epoch_withno_embeddings('test', testloader, ema, optimizer, criterion, args, aug=False)
        robust_loss_test, robust_acc_test = epoch_withno_embeddings('robust_test', robust_testloader, ema, optimizer, criterion, args, aug=False)
        
        mean, min = cluster_loss(config, testloader, ema, args, n_clusters = 10, random_state = 0)
    acc_test_list.append(acc_test)
    loss_test_list.append(loss_test)

    robust_acc_test_list.append(robust_acc_test)
    robust_loss_test_list.append(robust_loss_test)

    cluster_loss_min_list.append(min)
    cluster_loss_mean_list.append(mean)
    print("TestAcc Epoch {}:\t{}".format(ep, acc_test))
    print("cluster_loss_min Epoch {}:\t{}".format(ep, min))
    print("cluster_loss_mean Epoch {}:\t{}".format(ep, mean))
    ...
    if robust_acc_test > robust_acc_test_max:
        robust_acc_test_max = robust_acc_test
        robust_acc_test_max_epoch = ep
        print("NewMax {} at epoch {}".format(robust_acc_test_max, robust_acc_test_max_epoch))
    ...
    if return_loss:
        ...
        return net, acc_train_list, acc_test_list, cluster_loss_mean_list, cluster_loss_min_list, robust_acc_train_list, robust_acc_test_list
...
def get_eval_pool(eval_mode, model, model_eval):
    ...
    elif eval_mode == "CIFAR" or eval_mode == "SVHN":
