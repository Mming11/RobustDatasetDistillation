...
class ImageFolder_subset(datasets.DatasetFolder):
    def __init__(self,
                 root,
                 transform=None,
                 target_transform=None,
                 ...
                 seed=-1):
        self.extensions = IMG_EXTENSIONS if is_valid_file is None else None
        super(ImageFolder_subset, self).__init__(root,
                                          loader,
                                          self.extensions,
                                          transform=transform,
                                          target_transform=target_transform,
    ...
    def find_subclasses(self, nclass=100, phase=0, seed=0, subset='a'):
        """Finds the class folders in a dataset.
        """
        classes = []
        phase = max(0, phase)
        ...
        cls_to = nclass * (phase + 1)
        if seed == 0:
            with open(f'./misc/class_{subset}.txt', 'r') as f:
                class_name = f.readlines()
            for c in class_name:
                c = c.split('\n')[0]
                classes.append(c)
