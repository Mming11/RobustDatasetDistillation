...
def eval_loop(latents=None, f_latents=None, label_syn=None, G=None, best_acc={}, robust_best_acc={}, best_std={}, robust_testloader=None, testloader=None, model_eval_pool=[], it=0, channel=3, num_classes=10, im_size=(32, 32), args=None):
    curr_acc_dict = {}
    cluster_loss_dict = {}
    cluster_loss_min_dict = {}
    max_acc_dict = {}

    robust_curr_acc_dict = {}
    robust_max_acc_dict = {}
    ...
    for model_eval in model_eval_pool:
        ...
        for it_eval in range(args.num_eval):
            ...
            _, acc_train, acc_test, test_cluster_loss, test_cluster_loss_min, robust_acc_train, robust_acc_test = evaluate_synset(it_eval, net_eval, image_syn_eval, label_syn_eval, robust_testloader, testloader,
                                                     args=args, aug=True)
            del _
            del net_eval
            ...
            robust_accs_test.append(robust_acc_test)
            robust_accs_train.append(robust_acc_train)

            test_clusters_loss.append(test_cluster_loss)
            test_clusters_loss_min.append(test_cluster_loss_min)

        accs_test = np.array(accs_test)
        accs_train = np.array(accs_train)

        robust_accs_test = np.array(robust_accs_test)
        robust_accs_train = np.array(robust_accs_train)

        test_clusters_loss = np.array(test_clusters_loss)
        test_clusters_loss_min = np.array(test_clusters_loss_min)

        test_clusters_loss_mean = np.mean(np.max(test_clusters_loss, axis=1))
        test_clusters_loss_min_mean = np.mean(np.max(test_clusters_loss_min, axis=1))
        ...
        robust_acc_test_mean = np.mean(np.max(robust_accs_test, axis=1))
        robust_best_dict_str = "{}".format(model_eval)
        if robust_acc_test_mean > robust_best_acc[robust_best_dict_str]:
            robust_best_acc[robust_best_dict_str] = robust_acc_test_mean
            save_this_it = True

        curr_acc_dict[best_dict_str] = acc_test_mean
        curr_std_dict[best_dict_str] = acc_test_std

        robust_curr_acc_dict[robust_best_dict_str] = robust_acc_test_mean

        cluster_loss_dict[best_dict_str] = test_clusters_loss_mean
        cluster_loss_min_dict[best_dict_str] = test_clusters_loss_min_mean

        max_acc_dict[best_dict_str] = best_acc[best_dict_str]
        max_std_dict[best_dict_str] = best_std[best_dict_str]

        robust_max_acc_dict[robust_best_dict_str] = robust_best_acc[robust_best_dict_str]

        print('Evaluate %d random %s, mean = %.4f std = %.4f\n-------------------------' % (
        len(accs_test[:, -1]), model_eval, acc_test_mean, np.std(np.max(accs_test, axis=1))))
        wandb.log({'Accuracy/{}'.format(model_eval): acc_test_mean}, step=it)
        wandb.log({'Robust_Accuracy/{}'.format(model_eval): robust_acc_test_mean}, step=it)
        wandb.log({'Cluster_Avg_Accuracy/{}'.format(model_eval): test_clusters_loss_mean}, step=it)
        wandb.log({'Cluster_Min_Accuracy/{}'.format(model_eval): test_clusters_loss_min_mean}, step=it)
        wandb.log({'Max_Accuracy/{}'.format(model_eval): best_acc[best_dict_str]}, step=it)
        wandb.log({'Robust_Max_Accuracy/{}'.format(model_eval): robust_best_acc[robust_best_dict_str]}, step=it)
    ...
    wandb.log({
        ...
        'Robust_Accuracy/Avg_All'.format(model_eval): np.mean(np.array(list(robust_curr_acc_dict.values()))),

        'Cluster_Accuracy/Avg_All'.format(model_eval): np.mean(np.array(list(cluster_loss_dict.values()))),
        'Cluster_Min_Accuracy/Avg_All'.format(model_eval): np.mean(np.array(list(cluster_loss_min_dict.values()))),
        ...
        'Robust_Max_Accuracy/Avg_All'.format(model_eval): np.mean(np.array(list(robust_max_acc_dict.values()))),
